...
export default class SharedDocUtil extends SharedUtil
{
    ...
    addOpsToLookup(ops, clearFiles = false, haltOnError = false)
    {
        ...
        ops.forEach((op) =>
        {
            if (op && op.name && op.id)
            {
                ...
                const cachedName = cachedLookup.ids[op.id];
                if (cachedLookup.ids[op.id] && cachedName !== op.name)
                {
                    const opDocs = this.getOpDocsFromFile(cachedName);
                    if (opDocs && opDocs.id === op.id)
                    {
    ...
    _writeCache(cache, data)
    {
        ...
        case this.OP_LOOKUP_CACHE:
            this._storageUtil.writeJsonFileSync(this.opLookupFilename, data);
            break;
